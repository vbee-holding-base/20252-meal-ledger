...
const ParticipantManagement: React.FC = () => {
  ...
  const [targetParticipant, setTargetParticipant] = useState({ name: '', debt: 0 });

  const participants = [
    { id: '1', name: 'Việt Anh', debt: 100000, initial: 'A', bg: 'bg-primary-fixed', text: 'text-on-primary-fixed' },
    { id: '2', name: 'Đăng Khánh', debt: 0, initial: 'B', bg: 'bg-secondary-fixed', text: 'text-on-secondary-fixed' },
    { id: '3', name: 'Đình Vĩnh', debt: 120000, initial: 'C', bg: 'bg-tertiary-fixed', text: 'text-on-tertiary-fixed' },
    { id: '4', name: 'Nhật Minh', debt: 0, initial: 'D', bg: 'bg-outline-variant', text: 'text-on-surface-variant' },
    { id: '5', name: 'Kim Huy', debt: 50000, initial: 'H', bg: 'bg-primary-fixed-dim', text: 'text-on-primary-fixed-variant' },
  ];
  ...
  return (
    <div className="bg-surface text-on-surface min-h-screen flex flex-col font-body-md">
      {/* Top App Bar */}
      <header className="fixed top-0 w-full z-50 bg-surface flex items-center h-16 shadow-sm px-margin-mobile relative">
        <button 
          className="p-2 -ml-2 active:scale-95 transition-transform z-10" 
          onClick={() => navigate(-1)}
        >
          <span className="material-symbols-outlined text-primary">arrow_back</span>
        </button>
        <h1 className="absolute inset-0 flex items-center justify-center font-headline-md text-headline-md pointer-events-none" style={{ color: '#ff7a00' }}>
          Người tham gia
        </h1>
      ...
      <main className="flex-1 mt-16 pb-32">
        ...
        <section className="px-margin-mobile flex flex-col gap-3">
          ...
          <div className="flex items-center justify-between py-2">
            <h2 className="text-label-md font-label-md text-on-surface-variant">Tất cả người tham gia ({participants.length})</h2>
            <button className="text-primary font-label-sm text-label-sm flex items-center gap-1">
              Sắp xếp<span className="material-symbols-outlined text-base leading-none ml-1">unfold_more</span>
            </button>
          </div>
          ...
          <div className="flex flex-col gap-4">
            {participants.map((p) => (
              <div key={p.id} className="flex items-center justify-between p-4 bg-surface-container-lowest rounded-xl shadow-[0_4px_20px_rgba(153,71,0,0.06)] active:scale-[0.98] transition-all">
                <div className="flex items-center gap-4">
                  <div className={`w-12 h-12 rounded-full ${p.bg} flex items-center justify-center ${p.text} font-bold text-headline-md`}>
                    {p.initial}
                  </div>
                  <div>
      ...
      <div className={`${isAddModalOpen ? 'flex' : 'hidden'} fixed inset-0 z-[60] items-end`}>
        <div className="modal-overlay absolute inset-0" onClick={() => setIsAddModalOpen(false)}></div>
        <div className={`bg-surface-container-lowest w-full rounded-t-[32px] p-xl relative shadow-2xl transition-transform transform ${isAddModalOpen ? 'translate-y-0' : 'translate-y-full'} animate-slide-up`}>
          <div className="w-12 h-1.5 bg-outline-variant rounded-full mx-auto mb-8"></div>
          <h3 className="font-headline-md text-headline-md text-on-surface mb-6">Thêm người tham gia</h3>
          <div className="space-y-6">
            <div className="space-y-2">
              <label className="text-label-md font-label-md text-on-surface-variant">Tên hoặc Biệt danh</label>
              <input className="w-full h-14 px-4 rounded-2xl border-2 border-surface-container-high focus:border-primary focus:ring-0 text-body-md transition-all" placeholder="Nhập tên..." type="text" />
            </div>
      ...
      <div className={`${isDeleteModalOpen ? 'flex' : 'hidden'} fixed inset-0 z-[70] items-center justify-center px-margin-mobile`}>
        <div className="modal-overlay absolute inset-0" onClick={() => setIsDeleteModalOpen(false)}></div>
        <div className="bg-surface-container-lowest w-full max-w-sm rounded-[24px] p-md relative shadow-2xl overflow-hidden">
          <div className="flex flex-col items-center text-center space-y-4">
            ...
            <h3 className="font-headline-md text-headline-md text-on-surface">Bạn có chắc muốn xóa?</h3>
            <p className="text-body-md text-on-surface-variant">
              Xóa thành viên <strong>{targetParticipant.name}</strong> khỏi nhóm.
            </p>
            {/* Warning for debt */}
            {targetParticipant.debt > 0 && (
              <div className="flex items-start gap-3 p-3 bg-error-container/30 rounded-xl text-error text-left w-full">
                ...
                <p className="text-label-sm font-label-sm">
                  Người này vẫn còn nợ <strong>{(targetParticipant.debt / 1000)}k</strong>. Xóa sẽ làm mất dữ liệu khoản nợ này.
                </p>
              </div>
            )}
